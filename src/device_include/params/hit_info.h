...
__device__ inline void get_hitinfo(HitInfo& info, const GInstance* instance,
    const int prim_idx, const float2 uv, const float3 ray_dir, const int light_id)
{
    const GTriangleMesh* mesh = instance->mesh;
    ...
    if (mesh->normals)
        shading_normal = normalize(mesh->normals[index.x] * (1.0f - uv.x - uv.y) + mesh->normals[index.y] * uv.x + mesh->normals[index.z] * uv.y);
