...
void AssimpImporter::load_meshes()
{
    for(int i = 0; i < ascene->mNumMeshes; i++)
    {
        ...
        mesh->load_from_triangles(vertices, indices, normals, tangents, texcoords);
...
void AssimpImporter::traverse(aiNode* anode, shared_ptr<SceneGraphNode> node)
{
    ...
    for(int i = 0; i < anode->mNumMeshes; i++)
    {
        ...
        node->mesh_ids.push_back(mesh_id);
    }
...
void AssimpImporter::load_animations()
{
    for(int i = 0; i < ascene->mNumAnimations; i++)
    {
        ...
        for(int j = 0; j < aanim->mNumChannels; j++)
        {
            ...
            node->animation = make_shared<Animation>();
            ...
            for(int k = 0; k < achannel->mNumPositionKeys; k++)
            {
                ...
                node->animation->translations.push_back({time_stamp, translation});
            }
            for(int k = 0; k < achannel->mNumRotationKeys; k++)
            {
                ...
                float time_stamp = key.mTime / ticks_per_second;
                node->animation->rotations.push_back({time_stamp, rotation});
            }
            for(int k = 0; k < achannel->mNumScalingKeys; k++)
            {
                ...
                float time_stamp = key.mTime / ticks_per_second;
                node->animation->scales.push_back({time_stamp, scale});
            }
        }
    }
}